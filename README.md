...
This repository contains drivers for each & every major peripheral of this controller with different approach such as Polling based, Interrupt based & DMA integrated.
...
Every Driver in this repository has been implemented in real life on breadboard including all the necessary equipment for this.
For High level debugging and tracking the behavior of each peripheral closely, I have also used an 8-Channel Logic Analyzer. Captures are attached for each respective driver.
Software Used:
1. Keil μvision -> This is the primary compiler, used for its safety critical features for Raw C & according to the Industrial Standard.
...
5. Reading data from MPU6050 sensor (Accelerometer & Gyro) using I2C protocol based on Polling & DMA
6. SPI Loopback Driver by interconnecting MISO & MOSI based on Interrupt & DMA
7. Volume Knob System mimic in which the ADC values from a potentiometer manipulates the PWM duty cycle for an LED & use of Analog Watchdog to prevent upscaling further than a 75% threshold & a button override. 
