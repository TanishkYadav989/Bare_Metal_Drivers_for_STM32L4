...
Then, we initialise a Send & a Receive Function to map the size & data arguments to the Globally initialised Buffers & Variables & call the Start Functions inside them.
...
Two other functions exclusively for the MPU are initialised for Reading & Writing to the MPU,
1. MPU_Send -> We initialise an array containing the Register Address & its Value. Then this array is send to the MPU using the I2C_send function.
2. MPU_Receive -> We send Read Register Address to access the MPU's Data Register with a Read Condition & Start a Data Receive sequence to the Buffer alloted to the Function Argument.
