...
int fputc(int ch, FILE *f){                             //*Function to access printf/sprintf to read sensor data accurately*//
	(void)f;
	while(!(USART2->ISR & (1UL<<7)));                   //*Checks for Transfer Complete Reload Flag*//
	USART2->TDR=ch;
	return ch;
}