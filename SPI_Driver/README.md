...
For Configuring the SPI, we enable its clock & reset the whole peripheral to remove any Stale bits.
...
As it is a synchronous full duplex SCP, a single read & write function is initialized in which we map the function argument buffers to the Globally initialized Buffers & variables for index & size. After this TXEIE & RXEIE is set to start the Conversation.
