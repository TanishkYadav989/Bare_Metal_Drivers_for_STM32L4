*In the terminal capture we can see that the data is shifted towards right. This is a byte offset that usually occurs when use loopback system with automated data transfer with DMA. It is an Errata for the STM32L4 series & won't occur in generalized usage*
...
3. The DMA CH3's ISR is same as the SPI1's, it only checks for the TE flag & gives visual feedback of the error using the LED.
